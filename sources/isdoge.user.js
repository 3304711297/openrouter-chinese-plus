...
// @version      0.1.9
// @description  中文化 OpenRouter 管理控制台界面的部分菜单及内容
// @author       狗带带子
...
(function () {
  ...
  const EXACT_TEXT = new Map([
    ...
    ["Live LLM rankings based on benchmarks and real data from millions of people using models through OpenRouter.", "基于基准测试与数百万用户通过 OpenRouter 使用模型的真实数据，实时展示大语言模型排行榜。"],
    ["Shown are the sum of prompt and completion tokens per model, including reasoning tokens. Open models are those with publicly available weights.", "显示每个模型的提示词与补全 Token 总和，包含推理 Token。开放模型指权重公开可用的模型。"],
    ["Top models by task", "按任务排名的热门模型"],
    ["Each task's leading models, ranked by share of spend on OpenRouter", "各任务领先模型，按在 OpenRouter 上的支出份额排名"],
    ["Tasks are inferred by a classifier on a representative sample of prompts. Percentages show share of sample-corrected spend; deltas compare to the prior window.", "任务通过对代表性提示词样本运行分类器推断。百分比表示经样本校正后的支出份额；变化量与上一周期对比。"],
    ["Classification", "分类"],
    ["Compare text request share by model author on OpenRouter.", "按模型作者比较 OpenRouter 上的文本请求份额。"],
    ["Benchmarks", "基准测试"],
    ["Top models on OpenRouter by Artificial Analysis Intelligence Index", "OpenRouter 上按 Artificial Analysis 智能指数排名的热门模型"],
    ["Data sourced from Artificial Analysis", "数据来源：Artificial Analysis"],
    ["Fastest models", "最快模型"],
    ["Compare model performance across providers on OpenRouter", "比较 OpenRouter 上各提供商的模型性能"],
    ["Models with 100K+ requests in the last 24 hours", "过去 24 小时内请求量超过 10 万的模型"],
    ["Calculations are approximate, with a representative 0.25% sample of all prompts classified.", "计算结果为近似值，基于全部提示词中 0.25% 的代表性样本进行分类。"],
    ["Explore apps and agents", "探索应用与 Agent"],
    ["opting into usage tracking", "选择加入用量统计"],
    ["by category", "按分类"],
    ["Image Model Rankings", "图像模型排行榜"],
    ["Live image generation rankings based on real usage data from millions of people using models through OpenRouter.", "基于数百万用户通过 OpenRouter 使用模型的真实数据，实时展示图像生成排行榜。"],
    ["Weekly image-generating request volume across OpenRouter", "OpenRouter 上每周图像生成请求量"],
    ["Image Leaderboard", "图像排行榜"],
    ["Compare image models by image-generating request volume on OpenRouter.", "按图像生成请求量在 OpenRouter 上比较图像模型。"],
    ["Compare image-generating request share by model author on OpenRouter.", "按模型作者比较 OpenRouter 上的图像生成请求份额。"],
    ["Image Output", "图像输出"],
    ["Total images generated on OpenRouter", "OpenRouter 生成的图像总数"],
    ["Embedding Model Rankings", "嵌入模型排行榜"],
    ["Live rankings based on real usage data from people producing vector embeddings through OpenRouter.", "基于用户通过 OpenRouter 生成向量嵌入的真实使用数据，实时展示排行榜。"],
    ["Weekly embedding request volume across OpenRouter", "OpenRouter 上每周嵌入请求量"],
    ["Embedding Leaderboard", "嵌入排行榜"],
    ["Compare embedding models by request count on OpenRouter.", "按请求数在 OpenRouter 上比较嵌入模型。"],
    ["Shown are embedding requests per model.", "显示每个模型的嵌入请求数。"],
    ["Compare embedding request share by model author on OpenRouter.", "按模型作者比较 OpenRouter 上的嵌入请求份额。"],
    ["Rerank Model Rankings", "重排模型排行榜"],
    ["Live rankings based on real usage data from people reranking query results through models on OpenRouter.", "基于用户通过 OpenRouter 上的模型重排查询结果的真实使用数据，实时展示排行榜。"],
    ["Weekly rerank request volume across OpenRouter", "OpenRouter 上每周重排请求量"],
    ["Rerank Leaderboard", "重排排行榜"],
    ["Compare rerank models by request count on OpenRouter.", "按请求数在 OpenRouter 上比较重排模型。"],
    ["Shown are rerank requests per model.", "显示每个模型的重排请求数。"],
    ["Compare rerank request share by model author on OpenRouter.", "按模型作者比较 OpenRouter 上的重排请求份额。"],
    ["Video Model Rankings", "视频模型排行榜"],
    ["Live video generation rankings based on real usage data from millions of people using models through OpenRouter.", "基于数百万用户通过 OpenRouter 使用模型的真实数据，实时展示视频生成排行榜。"],
    ["Weekly video request volume across OpenRouter", "OpenRouter 上每周视频请求量"],
    ["Video Leaderboard", "视频排行榜"],
    ["Compare video models by request count on OpenRouter.", "按请求数在 OpenRouter 上比较视频模型。"],
    ["Shown are video model requests per model.", "显示每个模型的视频请求数。"],
    ["Compare video request share by model author on OpenRouter.", "按模型作者比较 OpenRouter 上的视频请求份额。"],
    ["Speech Model Rankings", "语音模型排行榜"],
    ["Live speech generation rankings based on real usage data from millions of people using models through OpenRouter.", "基于数百万用户通过 OpenRouter 使用模型的真实数据，实时展示语音生成排行榜。"],
    ["Weekly speech request volume across OpenRouter", "OpenRouter 上每周语音请求量"],
    ["Speech Leaderboard", "语音排行榜"],
    ["Compare text-to-speech models by request count on OpenRouter.", "按请求数在 OpenRouter 上比较文本转语音模型。"],
    ["Shown are text-to-speech requests per model.", "显示每个模型的文本转语音请求数。"],
    ...
    ["Live transcript generation rankings based on real usage data from millions of people using models through OpenRouter.", "基于数百万用户通过 OpenRouter 使用模型的真实数据，实时展示转录生成排行榜。"],
    ["Weekly transcription request volume across OpenRouter", "OpenRouter 上每周转录请求量"],
    ["Transcription Leaderboard", "转录排行榜"],
    ["Compare speech-to-text models by request count on OpenRouter.", "按请求数在 OpenRouter 上比较语音转文本模型。"],
    ["Shown are transcription requests per model.", "显示每个模型的转录请求数。"],
    ...
    ["Weekly speech-to-text transcript characters returned on OpenRouter by model.", "OpenRouter 上各模型每周返回的语音转文本转录字符数。"],
    ["We created this workspace for you with all your existing API keys, routing rules, privacy settings, guardrails, and configurations.", "我们已为你创建工作区，包含你现有的所有 API 密钥、路由规则、隐私设置、护栏和配置。"],
    ["Introducing Workspaces!", "工作区功能上线！"],
    ["Organize your OpenRouter usage into different environments with separate keys, configurations, budgets, and rules.", "将 OpenRouter 使用划分到不同环境，各自拥有独立的密钥、配置、预算和规则。"],
    ["This Week's Usage", "本周用量"],
    ["No spend this week", "本周无支出"],
    ["No requests this week", "本周无请求"],
    ["No tokens this week", "本周无 Token"],
    ...
    ["Connect monitoring tools like Langfuse or Datadog to track usage.", "连接 Langfuse 或 Datadog 等监控工具以跟踪用量。"],
    ["Create and manage API keys in this workspace.", "在此工作区创建并管理 API 密钥。"],
    ["Edit the workspace name and description.", "编辑工作区名称和描述。"],
    ["Set spending limits for API keys. This budget will apply to each API key assigned.", "为 API 密钥设置支出限额。此预算将应用于每个分配的 API 密钥。"],
    ["Credit Limit", "余额限额"],
    ["Include BYOK spend", "包含 BYOK 支出"],
    ["Leave blank for no limit.", "留空表示不设限额。"],
    ["This budget will apply individually to each key with this guardrail assigned. For example, if you set a budget of $100 weekly, each assigned key will have a $100 weekly budget.", "此预算会分别应用于分配了该护栏的每个密钥。例如，若设置每周 $100 预算，则每个分配的密钥都有 $100 的每周预算。"],
    ["Control which models and providers are available for this guardrail.", "控制此护栏可用的模型和提供商。"],
    ["Data Training", "数据训练"],
    ["These settings are separate from Zero Data Retention. Each endpoint toggle controls routing to endpoints with a specific data policy: enabling allows requests to be routed to those endpoints, disabling excludes them from routing.", "这些设置与零数据保留（ZDR）相互独立。每个端点开关控制是否路由到具有特定数据策略的端点：启用表示允许路由到这些端点，禁用表示从路由中排除。"],
    ["Allow paid endpoints that train on request data", "允许使用请求数据训练的付费端点"],
    ["Enable providers that may anonymously use data for training purposes.", "启用可能匿名使用数据进行训练的提供商。"],
    ["Allow free endpoints that train on request data", "允许使用请求数据训练的免费端点"],
    ["Enable providers serving free models that may retain and/or train on prompts and completions.", "启用提供免费模型且可能保留并/或训练提示词与补全内容的提供商。"],
    ["Allow free endpoints that publish prompts", "允许公开提示词的免费端点"],
    ["Enable providers serving free models that may publish prompts and completions to public datasets.", "启用提供免费模型且可能将提示词和补全内容发布到公共数据集的提供商。"],
    ["Access Policy", "访问策略"],
    ["Restriction Mode", "限制模式"],
    ["Only the providers and models listed below are accessible. Everything else is blocked.", "仅下列提供商和模型可访问，其余全部阻止。"],
    ["New providers and models will be automatically blocked.", "新提供商和模型将自动被阻止。"],
    ["Current Eligibility", "当前可用性"],
    ["Based on the restrictions, data policies, and org/workspace policies above.", "基于上述限制、数据策略以及组织/工作区策略。"],
    ["Classifiers", "分类器"],
    ["Tag every generation with structured metadata for AI usage reporting. View aggregated charts in the Activity page. Create up to 10 classifiers per workspace.", "为每次生成打上结构化元数据标签，用于 AI 用量报告。在活动页查看汇总图表。每个工作区最多创建 10 个分类器。"],
    ["No classifiers configured", "未配置分类器"],
    ["A classifier runs a cheap model over every generation and tags it with structured metadata you define — department, task type, sensitivity, etc. Tags power org-wide dashboards without exposing prompt content to leadership.", "分类器会对每次生成运行一个低成本模型，并打上你定义的结构化元数据标签——部门、任务类型、敏感度等。这些标签可支撑组织级看板，同时不向管理层暴露提示词内容。"],
    ["New classifier", "新建分类器"],
    ["Start from a preset, or make your own from scratch.", "从预设开始，或从零创建自己的分类器。"],
    ["Department", "部门"],
    ["Tag every request with the business unit or function it came from — engineering, sales, marketing, legal, etc.", "为每个请求打上来源业务单元或职能标签——工程、销售、市场、法务等。"],
    ["Audience", "受众"],
    ["Tag every request by who the output is being prepared for — internal use, client-facing, regulators, or the public.", "按输出面向对象给每个请求打标签——内部使用、面向客户、监管机构或公众。"],
    ["Engineering work", "工程工作"],
    ["Tag every request with the kind of engineering work it represents — feature dev, bug fixing, docs, refactoring, etc.", "为每个请求打上工程工作类型标签——功能开发、缺陷修复、文档、重构等。"],
    ["Agent complexity", "Agent 复杂度"],
    ["Classify agent prompts by difficulty tier and task family — understand whether agents are doing rote work or frontier-level reasoning.", "按难度层级和任务族对 Agent 提示词分类——了解 Agent 是在做重复性工作还是前沿级推理。"],
    ["Task type", "任务类型"],
    ["Classify every request by what the model is doing — coding, agent workflows, data processing, content writing, and more. Mirrors OpenRouter's internal task-type taxonomy.", "按模型正在执行的操作对每个请求分类——编码、Agent 工作流、数据处理、内容写作等。与 OpenRouter 内部任务类型分类体系一致。"],
    ["Capitalizable software expense", "可资本化软件支出"],
    ["Help accounting identify AI token spend on engineering work that qualifies as a capitalizable software development cost.", "帮助财务识别符合可资本化软件开发成本条件的工程类 AI Token 支出。"],
    ["Make your own", "自定义"],
    ["Custom classifier", "自定义分类器"],
    ["Start from scratch with your own dimensions", "从零开始，使用你自己的维度"],
    ["Test the classifier", "测试分类器"],
    ["Paste a sample user message and run the classifier against your draft schema. Uses your OpenRouter credits.", "粘贴示例用户消息，对你的草稿模式运行分类器。将消耗你的 OpenRouter 额度。"],
    ["Sample user message", "示例用户消息"],
    ["Draft a polite email asking the client for the latest quarterly figures.", "起草一封礼貌的邮件，向客户索取最新季度数据。"],
    ["Untitled classifier", "未命名分类器"],
    ["Classifier model", "分类器模型"],
    ["Strong accuracy on structured output, ~10× cheaper than Haiku.", "结构化输出精度高，成本约为 Haiku 的 1/10。"],
    ["Sample rate", "采样率"],
    ["of requests", "的请求"],
    ["Cost control. At high throughput, classifying every request adds up — drop to 10–25% if you only need rough usage shape rather than exact counts.", "成本控制。高吞吐量下，对每个请求分类会累积成本——若只需大致用量形态而非精确计数，可降至 10–25%。"],
    ["Dimensions", "维度"],
    ["No dimensions yet. Add one to get started.", "还没有维度。添加一个即可开始。"],
    [" · each becomes a tag on every generation", " · 每个维度都会成为每次生成的标签"],
    ["Classification prompt", "分类提示词"],
    ["Instructions sent to the classifier model as a system message.", "作为系统消息发送给分类器模型的指令。"],
    ["You don't need to list your dimensions or their values here.", "你无需在此列出维度或其取值。"],
    ["The output structure is enforced automatically — the model is constrained to emit one value from your enum for each dimension you defined above. An escape hatch is appended to every dimension so the model never has to invent a value.", "输出结构会自动强制执行——模型被约束为对你上面定义的每个维度，从其枚举中输出一个值。每个维度都会附加一个兜底选项，避免模型自行编造取值。"],
    ["Use the prompt for", "提示词用于"],
    [": framing (e.g. \"classify based on the user's intent, not the surface form of the words\"), edge-case guidance, and any domain context the model needs.", "：设定框架（例如“根据用户意图而非字面形式分类”）、边界情况指引，以及模型所需的领域上下文。"],
    ["Upstream Requests", "上游请求"],
    ["Routing of requests to upstream providers — does not include requests that failed prior to being sent upstream", "请求路由到上游提供商的记录——不包括发送上游前已失败的请求"],
    ["You don't have any video generations yet", "你还没有任何视频生成记录"],
    ["video generations", "视频生成"],
    ["Videos will show asynchronous video generations across statuses for the selected filters.", "视频标签页将显示所选筛选条件下各状态的异步视频生成记录。"],
    ["You don't have any batches yet", "你还没有任何批处理任务"],
    ["batches", "批处理"],
    ["Batches", "批处理"],
    ["Batches will show asynchronous batch jobs across statuses for the selected filters.", "批处理标签页将显示所选筛选条件下各状态的异步批处理任务。"],
    ["Prompt Injection Allowlist", "提示词注入允许列表"],
    ["Phrases that should never trigger the prompt injection guardrail. Matching is case-insensitive and exact.", "永远不应触发提示词注入护栏的短语。匹配不区分大小写且需完全匹配。"],
    ["Add phrases your users legitimately send that should not trigger the prompt injection guardrail. Matching is case-insensitive and exact.", "添加用户合法发送且不应触发提示词注入护栏的短语。匹配不区分大小写且需完全匹配。"],
    ["Allowlisted phrases only take effect when the", "允许列表中的短语仅在启用"],
    ["prompt injection guardrail", "提示词注入护栏"],
    ["is enabled for the request. Without it enabled, these patterns have no effect.", "时对该请求生效。未启用时，这些模式不会产生任何效果。"],
  ]);
